...
#      Small forms (k = 0..5) use a unary-style prefix of (k 1s followed by a 0)
...
#        [.... | xxx0]                                                   0    1       4       3       [        0 ..        7]
#        [xxxx | xx01]                                                   1    2       8       6       [        8 ..       71]
#        [xxxx | x011] [.... | xxxx]                                     2    3      12       9       [       72 ..      583]
#        [xxxx | 0111] [xxxx | xxxx]                                     3    4      16      12       [      584 ..     4679]
#        [xxx0 | 1111] [xxxx | xxxx] [xxxx | xxxx]                       4    6      24      19       [     4680 ..   528967]
#        [xx01 | 1111] [xxxx | xxxx] [xxxx | xxxx] [xxxx | xxxx]         5    8      32      26       [   528968 ..  528968+(1<<26)-1 ]
#
...
#      spec) is:
#         00111111 -> raw32 : 4 value bytes (little-endian)
#         01111111 -> raw40 : 5 value bytes
#         10111111 -> raw48 : 6 value bytes
#         11111111 -> raw64 : 8 value bytes
#
...
def bit_encode_small_posints(a: List[int]) -> bytes:
    ...
    for n in a:
        ...
        if n <= _TOP[0]:  # k=0 (1 nibble)
            ...
            emit_code(((n - _SHIFT[1]) << 2) | 0x01, 2)
        elif n <= _TOP[2]:  # k=2 (3 nibbles)
            emit_code(((n - _SHIFT[2]) << 3) | 0x03, 3)
        elif n <= _TOP[3]:  # k=3 (4 nibbles)
            emit_code(((n - _SHIFT[3]) << 4) | 0x07, 4)
        elif n <= _TOP[4]:  # k=4 (6 nibbles)
            emit_code(((n - _SHIFT[4]) << 5) | 0x0F, 6)
        elif n <= _TOP[5]:  # k=5 (8 nibbles)
            emit_code(((n - _SHIFT[5]) << 6) | 0x1F, 8)
        # Raw forms
        elif n <= 0xFFFFFFFF:  # raw32 (4 bytes)
            emit_code(0x00 | 0x3F, 2)
            emit_code(n, 8)
        elif n <= 0xFFFFFFFFFF:  # raw40 (5 bytes)
            emit_code(0x40 | 0x3F, 2)
            emit_code(n, 10)
        elif n <= 0xFFFFFFFFFFFF:  # raw48 (6 bytes)
            emit_code(0x80 | 0x3F, 2)
            emit_code(n, 12)
        else:  # raw64 (8 bytes)
            emit_code(0xC0 | 0x3F, 2)
            emit_code(n, 16)
...
def bit_decode_small_posints(data: bytes, count: int) -> List[int]:
    ...
    while len(res) < count:
        ...
        if not preview & 0x01:  # k=0 (1 nibble)
            res.append(_SHIFT[0] + (read_nibbles(1) >> 1))
        elif not preview & 0x02:  # k=1 (2 nibbles)
            res.append(_SHIFT[1] + (read_nibbles(2) >> 2))
        elif not preview & 0x04:  # k=2 (3 nibbles)
            res.append(_SHIFT[2] + (read_nibbles(3) >> 3))
        elif not preview & 0x08:  # k=3 (4 nibbles)
            res.append(_SHIFT[3] + (read_nibbles(4) >> 4))
        elif not preview & 0x10:  # k=4 (6 nibbles)
            res.append(_SHIFT[4] + (read_nibbles(6) >> 5))
        elif not preview & 0x20:  # k=5 (8 nibbles)
            res.append(_SHIFT[5] + (read_nibbles(8) >> 6))
        else:  # raw
            # Skip byte
            byte_index += 1
            # {4: 0xC, 5: 0x4, 6: 0x8, 8: 0x0}
            if preview == (0x00 | 0x3F):  # raw32
                res.append(read_nibbles(8))
            elif preview == (0x40 | 0x3F):  # raw40
                res.append(read_nibbles(10))
            elif preview == (0x80 | 0x3F):  # raw48
                res.append(read_nibbles(12))
            elif preview == (0xC0 | 0x3F):  # raw64
                res.append(read_nibbles(16))
            else:
...
if __name__ == "__main__":
    # Positive integer basic set
    print(f"\n    Positive integers (first 300)\n{'Number':<10} {'Hex':<20} {'Bin':<30}  {'Decoded'}")
    for i in range(300):
        enc = bit_encode_small_posints([i])
        dec = bit_decode_small_posints(enc, 1)[0]
    ...
    boundaries = [0,7,8,71,72,583,584,4679,4680,528967,528968, _MAX_SMALL, _MAX_SMALL+1,
                  0xFFFFFFFF, 0x100000000, 0xFFFFFFFFFFFF, 0x1000000000000, 0xFFFFFFFFFFFFFFFF]
    for v in boundaries:
        enc = bit_encode_small_posints([v])
        dec = bit_decode_small_posints(enc, 1)[0]
    ...
    enc = bit_encode_small_posints(arr)
    dec = bit_decode_small_posints(enc, len(arr))
    ...
    for i in range(-100, 101):
        enc = bit_encode_small_posints([encode_signed_as_unsigned(i)])
        dec_any = decode_unsigned_as_signed(bit_decode_small_posints(enc, 1)[0])
    ...
    for i in range(0, 11):
        arr = [i * (i + 1) // 2, i]
        enc = bit_encode_small_posints(arr)
        dec = bit_decode_small_posints(enc, 2)
    ...
    for i in range(1, 11):
        arr = list(range(i))
        enc = bit_encode_small_posints(arr)
        dec = bit_decode_small_posints(enc, i)
