...
cd `dirname -- ${BASH_SOURCE[0]}`/output || exit $?
...
plot_graph() {
    ...
    local PLOTS=""
    local N=1
    for i in $WANT_COLS; do
        PLOTS="$PLOTS'$name.txt' using $XAXIS(\$$(idx_of $i $ALL_COLS)-(($N.0-($N_COLS.0+1.0)/2.0)*0.03)*$RANGE.0/10.0) with lines ls $N title '$i', "
        N=$((N+1))
    done
