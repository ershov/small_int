...
def bit_decode_small_posints(data: bytes, count: int) -> list[int]:
    """Decode a byte sequence produced by bit_encode_small_posints."""
    result: list[int] = []
    n = 0
    shift = 0
...
Bin dump: {' '.join(f'{b:08b}' for b in encoded)}""")
    ...
    def encoded_len(v: int) -> int:
        return len(bit_encode_small_posints([v]))
    ...
    enc = bit_encode_small_posints(random_values)
    dec = bit_decode_small_posints(enc, len(random_values))
    ...
    for arr in mixed_arrays:
        ...
        dec = bit_decode_small_posints(enc, len(arr)) if arr else []
    ...
    enc_big = bit_encode_small_posints([big_value])
    assert len(enc_big) > 1
    truncated = enc_big[:-1]
    try:
        bit_decode_small_posints(truncated, 1)
    ...
    enc_two = bit_encode_small_posints([10, 20])
    try:
        bit_decode_small_posints(enc_two, 3)
    ...
    enc = bit_encode_small_posints([(1 << 63) - 1])
    dec = bit_decode_small_posints(enc, 1)[0]
